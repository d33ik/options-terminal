...
def prune_sessions(asset: str, keep: int = 20) -> int:
    ...
    # VACUUM — тільки поза транзакцією, інакше SQLite кидає
    # "cannot VACUUM from within a transaction". Тому окреме з'єднання
    # з isolation_level=None (автокоміт). Не критично: якщо не вдалось —
    # місце просто не звільниться, дані цілі.
    try:
        ...
        logger.warning(f"VACUUM пропущено: {e}")
