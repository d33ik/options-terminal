...
def main():
    ...
    print("  Eurex: %s   БД: %s" % (on_eurex, trade_date or "—"))

    if db_data and on_eurex and trade_date >= on_eurex:
        print("  Дані за %s актуальні" % trade_date)
        fetch = False
    elif on_eurex and (not db_data or on_eurex > trade_date):
        print("  Нова сесія %s — завантажую" % on_eurex)
        fetch = True
    else:
        fetch = not bool(db_data)
